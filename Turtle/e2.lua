...
local toDepth = tonumber(tArgs[2])
...
function goTo( x, y, z, xd, zd )
	while depth > y do
		if goTowardsStart() then
			depth = depth - 1
		elseif digStart() or attackStart() then
			collect()
		else
			sleep( 0.5 )
		end
	...
	while depth < y do
		if goTowardsDepth() then
			depth = depth + 1
		elseif digDepth() or attackDepth() then
			collect()
		else
			sleep( 0.5 )
		end
